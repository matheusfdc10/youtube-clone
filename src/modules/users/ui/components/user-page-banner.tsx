...
export const UserPageBanner = ({ user }: UserPageBannerProps) => {
    ...
    const [isBannerUploaderModal, setIsBannerUploaderModal] = useState(false);
    
    return (
        <div className="relative group">
            <BannerUploadModal 
                userId={user.id}
                open={isBannerUploaderModal}
                onOpenChange={setIsBannerUploaderModal}
            />
            <div 
                className={cn(
                    "w-full max-h-[200px] h-[15vh] md:h-[25vh] bg-gradient-to-r from-gray-100 to-gray-200 rounded-xl",
                    user.bannerUrl ? "bg-cover bg-center" : undefined,
                )}
                style={{
            ...
            >
                {user.clerkId === userId && (
                    <Button
                        onClick={() => setIsBannerUploaderModal(true)}
                        type="button"
                        size="icon"
